...
async function loadRoutes() {
    ...
    try {
        ...
        tableBody.innerHTML = routes.map(createRouteRow).join("");
    } catch (error) {
        console.error(error);
        ...
        tableBody.innerHTML = `
            <tr>
                <td colspan="7"
                    ...
                    "
                >
                    ${escapeHtml(error.message || "Could not load routes.")}
                </td>
            </tr>
        `;
    }
...
function initCreateRouteForm() {
    const showBtn = document.getElementById("show-create-route-btn");

    const panel = document.getElementById("create-route-panel");
    ...
    document
        .getElementById("cancel-create-route")
    ...
    form.addEventListener("submit", async (event) => {
        ...
        try {
            ...
            const payload = {
                ...
                originSpaceportId: Number(formData.get("originSpaceportId")),
                destinationSpaceportId: Number(formData.get("destinationSpaceportId")),
            ...
            alert(error.message || "Could not create the route.");
        } finally {
...
async function loadSpaceportsIntoForm() {
    try {
        ...
        document.getElementById("cr-origin").innerHTML =
            `<option value="">Select origin...</option>` + options;

        document.getElementById("cr-destination").innerHTML =
            `<option value="">Select destination...</option>` + options;
    } catch (error) {
        console.error(error);
    }
}
