...
export async function initDashboardPage() {
    ...
    try {
        ...
        statsGrid?.setAttribute(
            "class",
            "admin-stats-grid admin-view-error"
        );
...
function renderUpcomingFlights(data) {
    ...
    const tableBody =
        document.querySelector(
            "#dashboard-upcoming-flights tbody"
        );
...
function renderNetworkMetrics(data) {
    ...
    metrics.forEach(metric => {
        const key =
            metric.getAttribute("data-network-metric");
